...
def delete_post(id: str, db: Session = Depends(get_db), current_user : int = Depends(oauth2.get_current_user)):
    ...
    if post_query.first() is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"post with id:{id} does not existed")
    
    #CHECK IF USER IS POST CREATOR
    if current_user.id != str(post.owner_id):
